...
class OU_Noise:
    ...
    def _sample(self):
        ...
        # ========== YOUR CODE HERE ==========
        noise = np.random.randn(self.action_space) * self.sigma
        momentum = self.theta * (self.mu - self.state)
        self.state += momentum + noise
        return self.state
        # ========== YOUR CODE ENDS ==========
    ...
    def noise(self, action: np.ndarray[float]):
        ...
        return min(max(action + noise, lb), ub)
...
class DDPG:
    def __init__(
        ...
        self.actor = actor_model(
            self.observation_space, self.env.action_space.shape[0], **actor_kwargs
        ).to(device)
        self.critic = critic_model(
            self.observation_space, self.env.action_space.shape[0], **critic_kwargs
        ).to(device)
    ...
    def play_episode(
        self, sigma: float = 0, return_frames: bool = False, seed: int = None, env=None
    ):
        ...
        with torch.no_grad():
            while not done:
                action = (
                    self.actor(torch.tensor(state).float().to(self.device).unsqueeze(0))
                    .cpu()
                    .numpy()[0]
                )
                if sigma > 0:
    ...
    def _train_one_batch(self, batch_size):
        ...
        with torch.no_grad():
            ...
            target_qvals = self.target_critic(next_states, next_actions).squeeze()
            y = rewards + self.gamma * target_qvals * (~dones)

        qvals = self.critic(states, actions).squeeze()
        critic_loss = self.loss_fn(input=qvals, target=y)
        ...
        pred_qvals = self.critic(states, pred_actions)
        actor_loss = -pred_qvals.mean()
    ...
    def train(
        ...
        for episode in range(episodes):
            ...
            while not done:
                ...
                action = self.OU_noise.noise(action.detach().cpu().numpy())
            ...
            if episode % print_every == 0:
                print(
                    f"Episode {episode}: Time: {time.time() - start_time}, Total Reward: {total_reward}, Q Loss: {Q_loss_total}, Actor Loss: {actor_loss_total}"
                )

            if episode % val_freq == val_freq - 1:
