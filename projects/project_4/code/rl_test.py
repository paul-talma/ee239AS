...
def test_wrapper(wrapper):
    try:
        ...
        assert np.allclose(test_outputs[0], s), (
            f"at {np.where(wrong_indexs)} expected {test_outputs[0][wrong_indexs]} but got {s[wrong_indexs]}"
        )
        ...
        for i in range(len(test_outputs) - 1):
            ...
            assert np.allclose(test_outputs[i + 1], s), (
                f"expected {test_outputs[i + 1]} but got {s}"
            )
        ...
        for i in range(100):
            ...
            if i % 30 == 29:
                ...
                for k in range(len(target_outputs)):
                    ...
                    assert check_same_torch(target_outputs[k], actual_outputs[k]), (
