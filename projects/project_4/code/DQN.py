...
class DQN:
    ...
    def train(
        ...
        for episode in range(n_episodes):
            ...
            if episode % validate_every == validate_every - 1:
                ...
                print(
                    "Validation Mean Reward: {} Validation Std Reward: {}".format(
        ...
        print(
            "Test Mean Reward: {} Test Std Reward: {}".format(mean_reward, std_reward)
        )
    ...
    def load_model(self, suffix: str = ""):
        self.model.load_state_dict(
            torch.load(os.path.join(self.save_path, f"model_{suffix}.pt"))
        )

    def _save(self, suffix: str = ""):
        torch.save(
            self.model.state_dict(), os.path.join(self.save_path, f"model_{suffix}.pt")
        )

    def play_episode(
        self, epsilon: float = 0, return_frames: bool = True, seed: int = None
    ):
...
class HardUpdateDQN(DQN):
    def __init__(
        self, env, model, model_kwargs: dict = {}, update_freq: int = 5, *args, **kwargs
    ):
    ...
    def _save(self, suffix: str = ""):
        ...
        torch.save(
            ...
            os.path.join(self.save_path, f"target_model_{suffix}.pt"),
        )
    ...
    def load_model(self, suffix: str = ""):
        ...
        self.target_model.load_state_dict(
            torch.load(os.path.join(self.save_path, f"target_model_{suffix}.pt"))
        )


class SoftUpdateDQN(HardUpdateDQN):
    def __init__(
        self, env, model, model_kwargs: dict = {}, tau: float = 0.01, *args, **kwargs
    ):
        super().__init__(env, model, model_kwargs, *args, **kwargs)
        self.tau = tau
